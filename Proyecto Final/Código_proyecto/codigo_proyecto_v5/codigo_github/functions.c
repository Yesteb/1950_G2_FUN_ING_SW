...
void buscarRepresentante() {
    ...
    if (fptr == NULL) {
        printf("No se ha encontrado el archivo de representantes!\n");
        return;
    }
    
    char criterio[32];
    printf("Ingrese el ID o Apellido del representante a buscar: ");
    scanf("%s", criterio);
...
void buscarPago() {
    ...
    char criterio[32];
    printf("Ingrese el Apellido del Representante o del Estudiante a buscar: ");
    scanf("%s", criterio);
    ...
    if (!encontrado) {
        printf("No se encontró ningún pago con el criterio '%s'.\n", criterio);
    }
