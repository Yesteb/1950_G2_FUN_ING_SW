...
void RegistroPadreFamilia(){
	...
	printf("Ingrese la calle principal de domicilio: (Escriba todo en mayusculas o minusculas)\n");
	fgets(registroPd.direccionCP, 32, stdin);
	registroPd.direccionCP[strcspn(registroPd.direccionCP, "\n")]= 0;
	printf("Ingrese la calle secundaria de domicilio: (Escriba todo en mayusculas o minusculas)\n");
	fgets(registroPd.direccionCS, 32, stdin);
	registroPd.direccionCS[strcspn(registroPd.direccionCS, "\n")]= 0;
	printf("Ingrese el sector del domicilio: (Escriba todo en mayusculas o minusculas y sin espacios)\n");
...
void modificarPadreFamilia(){
	...
	char apellidoBuscado[32], nombreRepresentadoBuscado[32], nombreBuscado[32];
	char nuevoApellido[32], nuevoNombre[32], nuevoRepresentado[32];
	int encontrado = 0;
	char linea[128];
	
	while(getchar()!= '\n' );
	
	printf("Ingrese el apellido del representante que desea encontrar (Escriba todo en mayusculas o minusculas)\n");
	fgets(apellidoBuscado, 32, stdin);
	apellidoBuscado[strcspn(apellidoBuscado, "\n")] = 0;
	printf("Ingrese el nombre del representante que desea encontrar (Escriba todo en mayusculas o minusculas)\n");
	fgets(nombreBuscado, 32, stdin);
	nombreBuscado[strcspn(nombreBuscado, "\n")] = 0;
	printf("Ingrese el nombre del representado que desea encontrar (Escriba todo en mayusculas o minusculas)\n");
	fgets(nombreRepresentadoBuscado, 32, stdin);
	nombreBuscado[strcspn(nombreBuscado, "\n")] = 0;
	
	//funcion en base a la informacion que se guarda en la varable linea, tiene que cumplirse que la informacion buscada por el usuario se encuentre en la linea que le corresponde a cada informacion ingresada
	while(fgets(linea, 128, fptr1)){
		if(strstr(linea, apellidoBuscado) && strstr(linea, "Apellido: ")&&
			strstr(linea, nombreBuscado) && strstr(linea, "Nombre: ")&&
			strstr(linea, nombreRepresentadoBuscado) && strstr(linea, "Nombre del representado: "))
			{ //La primera aparicion de la cadena de strings buscada y la primera aparicion de la cadena de caracteres "Apellido: ,Nombre: , Nombre del representado: " en la informacion guardada en la variable linea tiene que cumplirse para hacer el proceso             
			encontrado=1;
	/*	 do {
			printf("%s", linea);
